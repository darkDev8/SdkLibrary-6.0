...
public class ExternalTools {
	...
	public static String changeDirectoryPath(String currentPath, String path) {
		if (new OSTools().isWindows()) {
			return changeWindowsDirectory(currentPath, path);
		} else {
			return changeUnixDirectory(currentPath, path);
		}
	}

	private static String changeWindowsDirectory(String currentPath, String path) {
		try {
	...
	private static String changeUnixDirectory(String currentPath, String path) {
		try {
			List<String> drives = new ArrayList<>();
			drives.addAll(Arrays.asList(new OSTools().getPartitionsInfo("letter")));
	...
	public static String getFilePath(String currentPath, String path) {
		if (new OSTools().isWindows()) {
	...
	private static String getWindowsFilePath(String currentPath, String path) {
		try {
			if (path.contains("/")) {
				return null;
			}
			...
			if (Files.exists(Paths.get(path)) && Files.isRegularFile(Paths.get(path))) {
				...
				Path pth = Paths.get(currentPath.concat("/").concat(path));
	...
	private static String getUnixFilePath(String currentPath, String path) {
		try {
			if (path.contains("\\")) {
				return null;
			}
